...
(default is 2)
"""
function norm(sys::StateSpace{S,Val{:cont}}, ::Type{Val{2}}) where {S}
...
function norm(sys::StateSpace{S,Val{:disc}}, ::Type{Val{2}}) where {S}
  A, B, C, D = sys.A, sys.B, sys.C, sys.D
  if !isstable(sys)
    return Inf
  else
...
norm(sys::LtiSystem,::Type{Val{T}}) where {T} = norm(ss(sys),Val{T})
